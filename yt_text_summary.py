...
def transcribe_with_whisper(url, api_key):
    ...
    using pytube (pure Python) and transcribes it using Groq's Whisper model.
    It handles private, age-restricted, members-only and region-blocked videos.
    """

    from pytube import YouTube
    from pytube.exceptions import VideoPrivate, MembersOnly, AgeRestrictedError, VideoUnavailable
    import tempfile
    ...
    try:
        ...
        st.error("👑 This video is members-only. Access is restricted.")
    except AgeRestrictedError:
        st.error("🔞 This video is age-restricted and cannot be processed.")
    except VideoUnavailable:
        st.error("🌍 This video is unavailable or region-blocked.")
    except Exception as e:
        st.error(f"❌ Failed to process this YouTube video: {e}")
...
if submit:
    ...
    # Clean YouTube URL (playlist, si params, etc.)
